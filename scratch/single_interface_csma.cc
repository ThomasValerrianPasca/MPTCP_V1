...
main (int argc, char *argv[])
{
	...
	for (i=0;i<nCsma;i++)
	{
		for (j=0;j<1;j++)
		{
	std::cout<<"IP Address = "<<csmaInterfaces.GetAddress(i,j)<<std::endl;//" Additional interface IP= "<<additional_interface_ip.GetAddress(0,0)<<" Additional interface IP= "<<additional_interface_ip.GetAddress(1,0)<<std::endl;
		}
	}
	...
	MpTcpBulkSendHelper source("ns3::TcpSocketFactory", InetSocketAddress(Ipv4Address(csmaInterfaces.GetAddress(3,0)), port));
